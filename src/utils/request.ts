...
import { useMessageBox, useMessage } from '/@/hooks/tool/message';
import qs from 'qs';
...
service.interceptors.request.use(
    (config: AxiosRequestConfig) => {
        ...
        //租户主键ID
        const tenantId = Local.getTenantId();
...
const handleResponse = (response: AxiosResponse<any>) => {
    //执行错误
    if (!isEmpty(response.data.code) && response.data.code != 0) {
...
service.interceptors.response.use(handleResponse, (error) => {
    ...
    } else if (status != 200 && error.response.data.msg) {
