<template>
	<el-form size="large" class="login-content-form" ref="loginFormRef" :rules="loginRules" :model="loginForm" @keyup.enter="onSignIn">
		<el-form-item class="login-animation2" prop="mobile">
			<el-input text placeholder="请输入手机" v-model="loginForm.mobile" clearable autocomplete="off">
				<template #prefix>
...
const loginRules = reactive({
	mobile: [
		{ required: true, trigger: 'blur', validator: rule.mobilePhone },
		{
			required: true,
			trigger: 'blur',
			message: '请输入手机号',
...
const handleSendCode = async () => {
	try {
		...
		await sendMobileCode(loginForm.mobile);
