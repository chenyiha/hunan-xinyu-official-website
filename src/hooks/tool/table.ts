...
export function useTable(options?: BasicTableProps) {
	...
	const query = async () => {
		...
		if (state.pageList) {
			try {
				...
				//分页回调
				if (state.pageCallBack instanceof Function){
					state.pageCallBack();
				}
			} catch (err: any) {
				// 捕获异常并显示错误提示
	 ...
	 */
	const getDataList = (refresh?: any=false) => {
	...
	async function handleDelete(
		...
		let msg = await ElMessageBox.confirm(
			'是否确认删除"' + deleteNames(arr, ids, id, name) + '"的数据项？',
			"系统提示",
			{
